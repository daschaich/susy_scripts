...
static int f(const int *ndim, const cubareal *p,
  ...
  *ff = PISQ * cos(TWOPI * tr);
  tr = sin(PI * p[0]) * sin(PI * p[0]) + sin(PI * p[1]) * sin(PI * p[1])
     + sin(PI * p[2]) * sin(PI * p[2]) + sin(PI * p[3]) * sin(PI * p[3]);
  *ff /= tr;
  return 0;
}
// -----------------------------------------------------------------
...
int main(int argc, char *argv[]) {
  ...
  if (fail == 0)
    printf("\n\nSuccess after %dM evalutions\n", (int)(neval / 1e6));
  else
    printf("\n\nFailure after %dM evalutions\n", (int)(neval / 1e6));
