...
# Parse arguments: Four-component integer vector, L and Nt
if len(sys.argv) < 7:
  print "Usage:", str(sys.argv[0]), "<n_1> <n_2> <n_3> <n_4> <L> <Nt>"
  sys.exit(1)
n = [0, 0, 0, 0]
n[0] = int(sys.argv[1])
n[1] = int(sys.argv[2])
...
pi_ov_Nt  = pi / float(Nt)
twopiOvNt = 2.0j * pi / float(Nt)
    ...
    for p3 in range(L):
      for p4 in range(Nt):
        ...
        denom = (sin(pi_ov_L * p1))**2 + (sin(pi_ov_L * p2))**2 \
              + (sin(pi_ov_L * p3))**2 + (sin(pi_ov_Nt * p4))**2
        toSum += num / denom

# Overall square root and constant factor of sqrt(4pi^2 / 4V),
# Don't add any extra normalization (determinants cancel)
one_ov_rI = pi * sqrt(toSum / float(L**3 * Nt))
